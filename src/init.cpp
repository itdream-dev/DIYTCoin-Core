...
#include "config/diytube-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "diytube.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "diytubed.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DIYT and zDIYT money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 62548, 42133));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DIYT/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
                                   FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DIYT/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, diytube, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DIYT/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DIYT specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzdiyt=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDIYT minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-resetspentzerocoin", strprintf(_("*You can lose zDIYT if you run this before the spend is confirmed on the blockchain.*  Reset current zDIYT spends to match those on the blockchain.%s"), ""));
    strUsage += HelpMessageOpt("-resetmintzerocoin", strprintf(_("*You can lose zDIYT if you run this before the mint is confirmed on the blockchain.*  Reset current zDIYT mints to match those on the blockchain.%s"), ""));

//    strUsage += "  -anonymizediytubeamount=<n>     " + strprintf(_("Keep N DIYT anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 62541, 42132));
...
std::string LicenseInfo() {
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%s The Diytube Core Developers"), "")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Diytube is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Diytube Core is shutting down."));
...
#endif
    // Make sure only a single Diytube process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Diytube Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Diytube specific: zerocoin and spork DB's
                ...
                // Diytube: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    ...
                    if (nZerocoinStartHeight != 0) {
                        if (chainActive.Height() > nZerocoinStartHeight) {
                            RecalculateZDIYTMinted();
                            RecalculateZDIYTSpent();
                        }
                    }
                    RecalculateDIYTSupply(1);
                }
                ...
                // Diytube: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Diytube Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Diytube Core to complete") << "\n";
        ...
        bool fEnableZDIYTBackups = GetBoolArg("-backupzdiyt", true);
        pwalletMain->setZDIYTAutoBackups(fEnableZDIYTBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeDiytubeAmount = GetArg("-anonymizediytubeamount", 0);
//    if (nAnonymizeDiytubeAmount > 999999) nAnonymizeDiytubeAmount = 999999;
//    if (nAnonymizeDiytubeAmount < 2) nAnonymizeDiytubeAmount = 2;
    ...
    LogPrintf("Anonymize Diytube Amount %d\n", nAnonymizeDiytubeAmount);
    ...
    /* Denominations
       ...
       For example:
       1DIYT+1000 == (.1DIYT+100)*10
       10DIYT+10000 == (1DIYT+1000)*10
    */
