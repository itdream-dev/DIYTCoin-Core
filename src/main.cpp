...
#if defined(NDEBUG)
#error "Diytube cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAgeConsensus = 12 * 60; // 12 hours
int64_t nReserveBalance = 0;

/** Fees smaller than this (in uDIYT) are considered zero fee (for relaying and mining)
...

    {
        ...
        if(tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDIYT spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zDIYT spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zDIYT spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight) {
    ...
    // POW Year 0
    if (nHeight == 0) {
        nSubsidy = 5000000.00 * COIN;
    } else if (nHeight >= Params().RAMP_TO_BLOCK() && nHeight < 1051201) {
      nSubsidy = 50 * COIN;
    } else if (nHeight >= 1051201 && nHeight < 2102401) {
      nSubsidy = 25 * COIN;
    } else if (nHeight >= 2102401 && nHeight < 3153601) {
      nSubsidy = 12.5 * COIN;
    } else if (nHeight >= 3153601 && nHeight < 4204801) {
      nSubsidy = 6.25 * COIN;
    } else if (nHeight >= 4204801 && nHeight <= 4980000) {
      nSubsidy = 3.125 * COIN;
    } else {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount) {
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight > nLastPOWBlock) {
            ...
            int64_t mNodeCoins = nMasternodeCount * 40000 * COIN;
    ...
    if (nHeight < Params().RAMP_TO_BLOCK()) {
        ret = 0;
    } else if (nHeight <= nLastPOWBlock && nHeight >= Params().RAMP_TO_BLOCK()) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean) {
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Diytube
...
void ThreadScriptCheck() {
    RenameThread("diytube-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZDIYTMinted() {
    int nZerocoinStartHeight = GetZerocoinStartHeight();
...
void RecalculateZDIYTSpent() {
    ...
    while (true) {
        ...
        //Remove spends from zDIYT supply
...
bool RecalculateDIYTSupply(int nHeightStart) {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zDIYT with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDIYTSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew) {
    ...
    // If turned on AutoZeromint will automatically convert DIYT to zDIYT
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zDIYT spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDIYT serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp) {
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDIYT mints and %d zDIYT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived) {
    LogPrintf("**********ProcessMessage %s %u %d %d\n",  SanitizeString(strCommand), vRecv.size(), pfrom->GetId(), pfrom->nVersion);
    RandAddSeedPerfmon();
    ...
    if (strCommand == "version") {
        ...
        LogPrint("ProcessMessage version", "%d", pfrom->GetId());
        if (pfrom->nVersion != 0) {
        ...
        // Diytube: We use certain sporks during IBD, so check to see if they are
