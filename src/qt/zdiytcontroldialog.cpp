...
#include "ui_zdiytcontroldialog.h"
...
std::list<std::string> ZDiytControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDiytControlDialog::listMints;

ZDiytControlDialog::ZDiytControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDiytControlDialog),
    model(0) {
    ui->setupUi(this);
    listMints.clear();
...
ZDiytControlDialog::~ZDiytControlDialog() {
    delete ui;
}

void ZDiytControlDialog::setModel(WalletModel *model) {
    this->model = model;
    updateList();
}

//Update the tree widget
void ZDiytControlDialog::updateList() {
...
void ZDiytControlDialog::updateSelection(QTreeWidgetItem* item, int column) {
...
void ZDiytControlDialog::updateLabels() {
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDiyt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDiytControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDiytControlDialog::GetSelectedMints() {
...
void ZDiytControlDialog::ButtonAllClicked() {
    ui->treeWidget->blockSignals(true);
