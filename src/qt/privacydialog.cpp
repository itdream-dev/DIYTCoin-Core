...
PrivacyDialog::PrivacyDialog(QWidget* parent) : QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint | Qt::WindowCloseButtonHint),
    ...
    // "Spending 999999 zDIYT ought to be enough for anybody." - Bill Gates, 2017
    ui->zDIYTpayAmount->setValidator(new QDoubleValidator(0.0, 21000000.0, 20, this));
    ...
    ui->labelzDIYTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_22_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDIYT->setEnabled(false);
        ui->pushButtonMintzDIYT->setToolTip(tr("zDIYT is currently disabled due to maintenance."));

        ui->pushButtonSpendzDIYT->setEnabled(false);
        ui->pushButtonSpendzDIYT->setToolTip(tr("zDIYT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked() {
    ...
    if (dlg.exec()) {
        ...
        ui->zDIYTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDIYT_clicked() {
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    if (GetAdjustedTime() < GetSporkValue(SPORK_21_ENABLE_ZEROCOIN)) {
        QMessageBox::information(this, tr("Mint Zerocoin"), tr("Zerocoin functionality is not enabled on the Diytube network yet."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }

    if(GetAdjustedTime() > GetSporkValue(SPORK_22_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"), tr("zDIYT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDIYT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDIYT in ") +
...
void PrivacyDialog::on_pushButtonSpendzDIYT_clicked() {

    if (!walletModel || !walletModel->getOptionsModel() || !pwalletMain)
        return;

    if (GetAdjustedTime() < GetSporkValue(SPORK_21_ENABLE_ZEROCOIN)) {
        QMessageBox::information(this, tr("Spend Zerocoin"), tr("Zerocoin functionality is not enabled on the Diytube network yet."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDIYT
        sendzDIYT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDIYT
    sendzDIYT();
}

void PrivacyDialog::on_pushButtonZDIYTControl_clicked() {
    ZDiytControlDialog* zDiytControl = new ZDiytControlDialog(this);
    zDiytControl->setModel(walletModel);
    zDiytControl->exec();
}

void PrivacyDialog::setZDiytControlLabels(int64_t nAmount, int nQuantity) {
    ui->labelzDIYTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDIYT() {
    QSettings settings;

    // Double is allowed now
    double dAmount = ui->zDIYTpayAmount->text().toDouble();
    ...
    if(ui->payTo->text().isEmpty()) {
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Diytube Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zDIYT is requested
    ...
    if(!fWholeNumber && fMintChange) {
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DIYT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDIYT</b>";
    ...
    // use mints from zDIYT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDiytControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDiytControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdiyt selector in case it was used
    ZDiytControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDIYT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DIYT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDIYT Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zDIYTpayAmount->setText("0");
    ui->labelzDIYTSelected_int->setText("0");
...
void PrivacyDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDIYT </b>";
    ...
    CAmount diytBalance = balance - (immatureBalance + nLockedBalance);
    if (diytBalance < 0) diytBalance = 0;

    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDIYT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDIYT "));
    ui->labelzDIYTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, diytBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zDIYT </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zDIYT </b> ";
        switch (denom) {
        case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::showOutOfSyncWarning(bool fShow) {
    ui->labelzDIYTSyncStatus->setVisible(fShow);
}
