...
void OptionsModel::Init() {
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DIYT);
    ...
    if (!settings.contains("nAnonymizeDiytubeAmount"))
        settings.setValue("nAnonymizeDiytubeAmount", 1000);

    nAnonymizeDiytubeAmount = settings.value("nAnonymizeDiytubeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDiytubeAmount"))
        SoftSetArg("-anonymizediytubeamount", settings.value("nAnonymizeDiytubeAmount").toString().toStdString());
...
void OptionsModel::Reset() {
    ...
    settings.clear();
    resetSettings = true; // Needed in diytube.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const {
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDiytubeAmount:
            return QVariant(nAnonymizeDiytubeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDiytubeAmount:
            nAnonymizeDiytubeAmount = value.toInt();
            settings.setValue("nAnonymizeDiytubeAmount", nAnonymizeDiytubeAmount);
            emit anonymizeDiytubeAmountChanged(nAnonymizeDiytubeAmount);
            break;
        case CoinControlFeatures:
