...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DIYT at the same time
    ONLY_40000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDIYT send
enum ZerocoinSpendStatus {
    ZDIYT_SPEND_OKAY = 0,                            // No error
    ZDIYT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDIYT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDIYT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDIYT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDIYT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDIYT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDIYT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDIYT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDIYT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDIYT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDIYT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDIYT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDIYT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDIYT_SPENT_USED_ZDIYT = 14,                      // Coin has already been spend
    ZDIYT_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
class CWallet : public CCryptoKeyStore, public CValidationInterface {
  ...
  public:
    ...
    */
    ...
    void setZDIYTAutoBackups(bool fEnabled) {
  ...
  public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 40000 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 40000 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
