...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake) {
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zDIYT spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zDIYT serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DiytubeMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DiytubeMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake) {
    LogPrintf("DiytubeMiner started\n");
    //skyclean ----
    if (fProofOfStake){
      LogPrintf("DiytubeMiner started - staking\n");
    } else {
        LogPrintf("DiytubeMiner started - proof\n");
    }
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("diytube-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running DiytubeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
