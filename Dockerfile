...
COPY depends /Diytube/depends
ENV SDK_PATH=/Diytube/depends/SDKs/
RUN dpkg --add-architecture i386 \
  ...
  && wget https://github.com/phracker/MacOSX-SDKs/releases/download/10.13/MacOSX10.11.sdk.tar.xz -P /Diytube/depends/SDKs/ \
  && tar xJf /Diytube/depends/SDKs/MacOSX10.11.sdk.tar.xz -C /Diytube/depends/SDKs/ \
  && rm /Diytube/depends/SDKs/MacOSX10.11.sdk.tar.xz \
  && wget https://bootstrap.pypa.io/get-pip.py \
  && python get-pip.py \
  && rm get-pip.py \
  && pip install ez_setup==0.9 \
  && make -C /Diytube/depends HOST=arm-linux-gnueabihf \
  && make -C /Diytube/depends HOST=i686-pc-linux-gnu \
  && make -C /Diytube/depends HOST=i686-w64-mingw32 \
  && make -C /Diytube/depends HOST=x86_64-unknown-linux-gnu \
  && make -C /Diytube/depends HOST=x86_64-w64-mingw32

FROM dependencies AS base
COPY autogen.sh /Diytube/
COPY build-aux /Diytube//build-aux
COPY configure.ac /Diytube/
COPY contrib /Diytube//contrib
COPY Makefile.am /Diytube/
COPY pkg.m4 /Diytube/
COPY qa /Diytube/qa
COPY share /Diytube/share
COPY src /Diytube/src
WORKDIR /Diytube

FROM base AS arm32
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
COPY --from=dependencies /Diytube/depends/arm-linux-gnueabihf $(pwd)/depends/arm-linux-gnueabihf
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
  ...
  && make \
  && cp ./src/diytube-cli ./src/diytubed ./src/qt/diytube-qt / \
  && make clean
WORKDIR /
RUN arm-linux-gnueabihf-strip ./diytube-cli ./diytube-qt ./diytubed  \
  && tar czf arm32.tar.gz ./diytube* \
  && rm ./diytube-cli ./diytube-qt ./diytubed

FROM base AS linux32
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
COPY --from=dependencies /Diytube/depends/i686-pc-linux-gnu $(pwd)/depends/i686-pc-linux-gnu
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
  --with-pic \
  ...
  && make clean
WORKDIR /
RUN strip ./diytube-cli ./diytube-qt ./diytubed  \
  && tar czf linux32.tar.gz ./diytube* \
  && rm ./diytube-cli ./diytube-qt ./diytubed

FROM base AS linux64
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
ENV HOST="x86_64-unknown-linux-gnu"
COPY --from=dependencies /Diytube/depends/x86_64-unknown-linux-gnu $(pwd)/depends/x86_64-unknown-linux-gnu
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
  --with-pic \
...
WORKDIR /
RUN strip ./diytube-cli ./diytube-qt ./diytubed  \
  && tar czf linux64.tar.gz ./diytube* \
  && rm ./diytube-cli ./diytube-qt ./diytubed

FROM base AS windows32
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
ENV HOST="i686-w64-mingw32"
COPY --from=dependencies /Diytube/depends/i686-w64-mingw32 $(pwd)/depends/i686-w64-mingw32
COPY autogen.sh .
...
RUN ./autogen.sh \
  ...
  && make \
  && cp ./src/diytube-cli.exe ./src/diytubed.exe ./src/qt/diytube-qt.exe / \
  && make clean
WORKDIR /
RUN strip ./diytube-cli.exe ./diytube-qt.exe ./diytubed.exe  \
  && tar czf windows32.tar.gz ./diytube* \
  && rm ./diytube-cli.exe ./diytube-qt.exe ./diytubed.exe

FROM base AS windows64
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
ENV HOST="x86_64-w64-mingw32"
COPY --from=dependencies /Diytube/depends/x86_64-w64-mingw32 $(pwd)/depends/x86_64-w64-mingw32
COPY autogen.sh .
COPY build-aux ./build-aux
COPY configure.ac .
COPY contrib ./contrib
...
WORKDIR /
RUN strip ./diytube-cli.exe ./diytube-qt.exe ./diytubed.exe  \
  && tar czf windows64.tar.gz ./diytube* \
  && rm ./diytube-cli.exe ./diytube-qt.exe ./diytubed.exe

FROM alpine:3.8
LABEL maintainer="kewagi"
LABEL version="2.0.0.0"
LABEL name="Diytube Docker Builds"
RUN mkdir /release
